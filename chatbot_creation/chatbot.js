// chatbot.js - Functionality for the Secure AI Chatbot Interface (Click-to-Activate & Dynamic Load)

document.addEventListener('DOMContentLoaded', () => {

    const chatbotPlaceholder = document.getElementById('chatbot-placeholder');
    if (!chatbotPlaceholder) {
        console.error("Chatbot placeholder not found. Chatbot will not load.");
        return;
    }

    fetch('chatbot_creation/chatbot-snippet.html')
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status} while fetching chatbot-snippet.html`);
            }
        ...
        .then(html => {
            ...
            // All original script logic MUST now run AFTER the HTML is injected.
            ...
            const honeypotInput = document.getElementById('ops_contact_me_by_fax_only');
            ...
            // Guard against missing critical elements after load (should not happen if snippet is correct)
            if (!chatbotLauncher || !chatContainerWrapper || !aiChatbotContainer || !closeChatbotButton || !chatMessages || !userInput || !sendButton || !chatForm || !securityCheckContainer || !captchaPlaceholder) {
                console.error("One or more critical chatbot HTML elements not found after loading snippet. Chatbot may not function correctly.");
                return;
            }
